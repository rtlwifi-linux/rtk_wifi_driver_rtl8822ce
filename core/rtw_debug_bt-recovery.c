...
#if 1
	printk("BT recovery start....");	
	v8 = rtw_read8(padapter, 0x75);
	rtw_write8(padapter, 0x75, v8|BIT0);

	rtw_write8(padapter, 0xA3, 0);

	v32 = rtw_read32(padapter, 0xA0);
	rtw_write32(padapter, 0xA0, v32 & (~(BIT24|BIT25|BIT26)));
	
	v8 = rtw_read8(padapter, 0x2C);
	rtw_write8(padapter, 0x2C, (v8 & 0x0F) | 0xB0);

	
	v8 = rtw_read8(padapter, 0x6a);
	rtw_write8(padapter, 0x6a, v8 & (~BIT2));
	mdelay(500);
	rtw_write8(padapter, 0x6a, v8 | BIT2);

	start = rtw_get_current_time();
	do {
		rtw_write8(padapter, 0x2c, 0xB);
		mdelay(1);
	} while (rtw_get_passing_time_ms(start) < 2100);

	
	v8 = rtw_read8(padapter, 0x2c);
	rtw_write8(padapter, 0x2c, (v8 & 0x0F ) | 0x80);


	rtw_write8(padapter, 0xA3, 0x5);
