...
int rtw_get_sta_tx_stat(_adapter *adapter, struct sta_info *sta)
{
	...
	pstapriv_primary = &(GET_PRIMARY_ADAPTER(adapter))->stapriv;

	_rtw_memcpy(pstapriv_primary->c2h_sta_mac, sta->cmn.mac_addr, ETH_ALEN);
	pstapriv_primary->c2h_adapter_id = adapter->iface_id;
	rtw_sctx_init(&gotc2h, 60);
	pstapriv_primary->gotc2h = &gotc2h;
	rtw_hal_reqtxrpt(adapter, mac_id);
	if (rtw_sctx_wait(&gotc2h, __func__) == _FAIL) {
		RTW_WARN("Warning : Query timeout, operation abort!!\n");
		ret = -EFAULT;
	}
